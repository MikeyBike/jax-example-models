...
def main():
    ...
    stan_file = script_dir / "normal_mixture.stan"
    ...
    print(f"Posterior mean (theta): {theta_draws.mean():.4f}")
    print(f"Posterior std (theta): {theta_draws.std(ddof=1):.4f}")

    for i, label in enumerate(["lower", "upper"]):
        print(f"Posterior mean  (mu[{label}]): {mu_draws[:, i].mean():.4f}")
        print(f"Posterior std   (mu[{label}]): {mu_draws[:, i].std(ddof=1):.4f}")
