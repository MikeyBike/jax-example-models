...
def main():
    ...
    def normal_mixture_k_prop_model(y, K):
        ...
        mu_prop_bij  = tfb.SoftmaxCentered()
        mu_scale_bij = tfb.Exp()   
        sigma_bij    = tfb.Exp()  
        ...
        def log_density(position):
            ...
            mu_prop  = mu_prop_bij.forward(mu_prop_unc)     
            ...
            log_det_mu_prop  = mu_prop_bij.forward_log_det_jacobian(mu_prop_unc, event_ndims=1)
        ...
        def inv_transform(position):
            ...
            mu_prop  = mu_prop_bij.forward(position["mu_prop_unc"])
            mu_loc   = position["mu_loc"]
        ...
        def generate(rng, theta, mu, sigma):
            k1, k2     = jrd.split(rng)
            components = tfd.Categorical(probs=theta).sample(seed=k1, sample_shape=(N,))
    ...
    log_density, inv_transform, generate, initialize_random = (
        normal_mixture_k_prop_model(y, K)
    )
