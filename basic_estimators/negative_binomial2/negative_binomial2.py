...
def main():
    ...
    def negative_binomial_model(y):
        ...
        exp_bij = tfb.Exp()
        sigmoid_bij = tfb.Sigmoid()

        def log_density(position):
            alpha_unc = position["alpha_unconstrained"]
            p_success_unc = position["p_success_unconstrained"]

            alpha = exp_bij.forward(alpha_unc)
            p_success = sigmoid_bij.forward(p_success_unc)

            log_det_alpha = exp_bij.forward_log_det_jacobian(alpha_unc, event_ndims=0)
            log_det_p = sigmoid_bij.forward_log_det_jacobian(p_success_unc, event_ndims=0)

            lp = jnp.sum(tfd.NegativeBinomial(total_count=alpha, probs=1.0 - p_success).log_prob(y))
            lp += log_det_alpha + log_det_p
            return lp

        def inv_transform(position):
            alpha = exp_bij.forward(position["alpha_unconstrained"])
            p_success = sigmoid_bij.forward(position["p_success_unconstrained"])
        ...
        def generate(rng, alpha, p_success):
            return {
                "y_pred": tfd.NegativeBinomial(total_count=alpha, probs=1.0 - p_success).sample(seed=rng, sample_shape=(N,))
            }
        ...
        def initialize_random(rng):
            ...
            return {
                ...
                "p_success_unconstrained": jrd.normal(k2, shape=()),
            }
    ...
    def posterior_predictive_check(key, draws, generate_fn):
        ...
        return jax.vmap(generate_fn, in_axes=(0, 0, 0))(
            keys, draws["alpha"], draws["p_success"]
        )
