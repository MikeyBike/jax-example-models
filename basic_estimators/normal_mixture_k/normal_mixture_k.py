...
def main():
    ...
    def normal_mixture_k_model(y, K):
        theta_bij = tfb.SoftmaxCentered()
        sigma_bij = tfb.Chain([tfb.Scale(10.0), tfb.Sigmoid()])
        ...
        def log_density(position):
            ...
            theta = theta_bij.forward(theta_unc)
            sigma = sigma_bij.forward(sigma_unc)

            log_det_theta = theta_bij.forward_log_det_jacobian(theta_unc, event_ndims=1)
            log_det_sigma = sigma_bij.forward_log_det_jacobian(sigma_unc, event_ndims=1)

            lp = tfd.Normal(0.0, 10.0).log_prob(mu).sum()
            lp += tfd.Dirichlet(jnp.ones(K)).log_prob(theta)  # Attempt to copy the stick-breaking prior
            ...
            lp += log_det_theta + log_det_sigma
            return lp

        def inv_transform(position):
            theta = theta_bij.forward(position["theta_unc"])
            sigma = sigma_bij.forward(position["sigma_unc"])
            return {"theta": theta, "mu": position["mu"], "sigma": sigma}

        def generate(rng, theta, mu, sigma):
            k1, k2 = jrd.split(rng)
            components = tfd.Categorical(probs=theta).sample(
                seed=k1, sample_shape=(N,)
            )
            y_pred = tfd.Normal(mu[components], sigma[components]).sample(seed=k2)
            return {"y_pred": y_pred}

        def initialize_random(rng):
            k1, k2, k3 = jrd.split(rng, 3)
            return {
                "theta_unc": jrd.normal(k1, shape=(K - 1,)),
                "mu": jrd.normal(k2, shape=(K,)),
                "sigma_unc": jrd.normal(k3, shape=(K,)),
            }

        return log_density, inv_transform, generate, initialize_random

    log_density, inv_transform, generate, initialize_random = normal_mixture_k_model(y, K)
